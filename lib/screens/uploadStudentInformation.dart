...
class _UploadStudentInfoState extends State<UploadStudentInfo> {
  ...
  _showStudents() async{
    ...
    Alert(
      ...
      desc: "Would you like to save students info into the database?",
      buttons: [
        DialogButton(
          child: Text(
            "YES",
          ...
          onPressed: () async {
            _insertStudentsInfo(_data);
            if (_inserted)
              Alert(
                context: context,
                title: "Success!",
                desc: _student.length.toString() +
  ...
  _insertStudentsInfo(List<List<dynamic>> _data) async{
    for (int i=1; i<_data.length; i++) {
      try{
        ...
        Alert(
          context: context,
          title: "Erorr!",
          desc: "Students info. is already inserted or invalid file format",
          image: Image.asset("images/fail.png"),
        ).show();
